...
**PushIt** is a Python 3 app to use **Push2 as a standalone MIDI sequencer**. It would not exist without the amazing work of [ffont](https://github.com/ffont). Specifically [push2-python](https://github.com/ffont/push2-python) for the Push2 interface, and [pysha](https://github.com/ffont/pysha) for the app structure. To run PushIt, you just need to install Python requirements and run `app.py` on a computer connected to Push2 and with a MIDI interface to output messages.
...
PushIt **can run on a Raspberry Pi** (see instructions below) so you can use Push2 as a standalone controller without your laptop around. PushIt is based on [push2-python](https://github.com/ffont/push2-python). `push2-python` requires [pyusb](https://github.com/pyusb/pyusb) which is based in [libusb](https://libusb.info/). You'll most probably need to manually install `libusb` for your operative system if `pip install -r requirements.txt` does not do it for you. Moreover, to draw on Push2's screen, PushIt uses [`pycairo`](https://github.com/pygobject/pycairo) Python package. You'll most probably also need to install [`cairo`](https://www.cairographics.org/) if `pip install -r requirements.txt` does not do it for you (see [this page](https://pycairo.readthedocs.io/en/latest/getting_started.html) for info on that).

## Features

I designed PushIt (and I continue to update it) to serve my own specific setup needs, but hopefully it can be useful (or adapted!) to work on other setups as well. In my setup, I run PushIt on a Rapsberry Pi and connected to Push2. Push2 is used as my main source of MIDI input, and the generated MIDI is routed to a Squarp Pyramid sequencer. From there, Pyramid connects to all the other machines in the setup. Also, I have a MIDI keyboard connected to PushIt so that the notes generated from the keyboard are merges with the notes generated from Push. Below is a diagram of my setup with PushIt. These are the features that PushIt has currently implemented:
...
Well, this is not a proper user manual, but here are some notes about how to use PushIt:
 ...
 * Press `Setup` button several times to cycle through configuration pages where you'll find options to:
   ...
   * Save current settings to file (will be loaded automatically when PushIt runs again)
   * Run software update (to update PushIt version, requires internet connection)
 ...
 * Press `User` button to deactivate the display (useful for slow computers running PushIt).
 * Press `Add track` button to enter *Pyramid track triggering* mode (or hold the button to only momentarily activate that mode). While in this mode, you can mute/unmute the 64 Pyramid tracks using the 64 pads of the Push. Note that PushIt does not get information from Pyramid about the current status of tracks, therefore it might be out of sync with it. You can manually indicate that a track "has content" by pressing the corresponding pad, then you can mute/unmute that track by pressing the pad again. Long pressing one pad will set the corresponding track to "no content" state. In this way, you can manualy sync the track status in PushIt and the track status from Pyramid. Hopefully future Pyramid updated will allow to do this process automatically and provide tighter integration. 
...
## Instructions to get PushIt running on a RaspberryPi
...
```
git clone https://github.com/jasonbirchler/AhPushIt.git
```
...
Create file in `/lib/systemd/system/pushit.service`...

    sudo nano /lib/systemd/system/pushit.service
...
[Service]
WorkingDirectory=/home/pi/AhPushIt
ExecStart=/usr/bin/python3 /home/pi/AhPushIt/app.py                                                
